...
export class StoreItemsService {
  ...
  constructor(
    ...
    // âœ… dependency required for realtime events
  ...
  async createItem(input: {
    ...
    // âœ… FIX: only fields that exist on StoreItem
  ...
  > {
    ...
    // âœ… FIX: DB column is amountCents (entity maps priceCents -> amountCents)
  ...
  async createOrderForUser(
    ...
    if (item.venueId) {
      ...
      // âœ… FIX: venue-scoped item MUST have a real owner to receive payout
      if (!venueOwnerId) {
        throw new BadRequestException('Venue owner not found for this item.');
      }
    } else {
      // Global item (no venue) â€” you currently have no platform wallet payout target.
  ...
  async ownerCancelOrder(ownerId: number, orderId: number): Promise<any> {
    return this.findOwnerOrderByIdLive(ownerId, orderId);
  }

  async ownerMarkOrder(ownerId: number, orderId: number, status: string): Promise<any> {
    return this.findOwnerOrderByIdLive(ownerId, orderId);
  }
