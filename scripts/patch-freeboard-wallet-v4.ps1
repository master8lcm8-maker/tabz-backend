...
function TABZ_ReadUtf8([string]$Path) {
  return [System.IO.File]::ReadAllText((Resolve-Path $Path), [System.Text.Encoding]::UTF8)
}

function TABZ_WriteUtf8([string]$Path, [string]$Text) {
...
$fm = TABZ_ReadUtf8 $freeboardModule
...
$fs = TABZ_ReadUtf8 $freeboardService
