...
function TABZ_InsertAfter {
  param(
    ...
    # absorb any accidental/ghost named param like -Encoding
    [Parameter(Mandatory=$false)][string]$Encoding = "UTF8",
    [Parameter(ValueFromRemainingArguments=$true)][object[]]$Ignored
  )
...
function:TABZ_InsertAfter -Path $freeboardModule `
...
TABZ_WriteUtf8 $freeboardModule $fm2
}
...
function:TABZ_InsertAfter -Path $freeboardService `
...
TABZ_WriteUtf8 $freeboardService $fs2
}
